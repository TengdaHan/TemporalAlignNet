...
class Sentence_Embedding(nn.Module):
    ...
    def filter_words(self, sentences):
        filter_sentences = [] 
        for sent in sentences:
            words = [word for word in sent if word in self.word_to_token]
            filter_sentences.append(' '.join(words))
...
class S3D(nn.Module):
    def __init__(self, dict_path, num_classes=512, gating=True, space_to_depth=True):
        ...
        if dict_path is not None:
            ...
            print('[warning] not loading language model')
    ...
    def freeze_forward(self, inputs):
        """Defines the S3DG base architecture.
        """
        with th.no_grad():
            if self.space_to_depth:
